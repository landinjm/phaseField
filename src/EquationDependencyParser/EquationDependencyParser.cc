    ...
    std::vector<bool> & var_nonlinear)
{
    ...
    for (unsigned int i=0; i<var_name.size(); i++){
        ...
        if (var_eq_type[i] == EXPLICIT_TIME_DEPENDENT){
            ...
            parseDependencyListRHS(var_name, var_eq_type, i, sorted_dependencies_value_RHS.at(i), sorted_dependencies_gradient_RHS.at(i), need_value_explicit_RHS, need_gradient_explicit_RHS, need_hessian_explicit_RHS, need_value_residual_entry, need_gradient_residual_entry,single_var_nonlinear);
    ...
    for (unsigned int dep=0; dep<split_dependency_list.size(); dep++){
        ...
        for (unsigned int var=0; var<var_name.size(); var++){
            ...
            if (split_dependency_list.at(dep) == var_name.at(var)){
                ...
                dependency_entry_assigned = true;
                if (var_index != var){
                    std::cerr << "PRISMS-PF Error: Dependency entry " << split_dependency_list.at(dep) << " is not valid because the change in a variable can only be accessed in its own governing equation." << std::endl;
                    abort();
                }
            }
...
void EquationDependencyParser::pp_parse(
    std::vector<std::string> var_name,
    ...
    std::vector<std::string> sorted_dependencies_gradient)
{
    ...
    for (unsigned int i=0; i<pp_var_name.size(); i++){
        ...
        parseDependencyListPP(var_name,sorted_dependencies_value.at(i), sorted_dependencies_gradient.at(i), pp_need_value, pp_need_gradient, pp_need_hessian, need_value_residual_entry, need_gradient_residual_entry);
...
void EquationDependencyParser::parseDependencyListPP(std::vector<std::string> var_name, std::string value_dependencies, std::string gradient_dependencies, std::vector<bool> & need_value, std::vector<bool> & need_gradient, std::vector<bool> & need_hessian, bool & need_value_residual, bool & need_gradient_residual){
