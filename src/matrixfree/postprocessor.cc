	...
	for(unsigned int fieldIndex=0; fieldIndex<userInputs.pp_number_of_variables; fieldIndex++){
		...
		if (userInputs.pp_varInfoList[fieldIndex].is_scalar){
			...
			*U=0;
			...
			pcout << "Vector postprocess variables are currently not supported" << std::endl;
			exit(-1);
		}
...
void MatrixFreePDE<dim,degree>::getPostProcessedFields(const dealii::MatrixFree<dim,double> &data,
	...
	variableContainer<dim,degree,dealii::VectorizedArray<double> > pp_variable_list(data,userInputs.pp_varInfoList,scalarField);
