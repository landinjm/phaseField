  ...
  [[maybe_unused]] Point<dim, VectorizedArray<double>> q_point_loc) const
{
  ...
  // Normal vector
  scalargradType n = phix / (std::sqrt(phix.norm_square()) + 1e-10);

  // 6-fold cos(theta)
  scalarvalueType cos_6_theta = constV(1.0) -
                                constV(18.0) * dealii::Utilities::fixed_power<2>(n[1]) +
                                constV(48.0) * dealii::Utilities::fixed_power<4>(n[1]) -
                                constV(32.0) * dealii::Utilities::fixed_power<6>(n[1]);

  // Anisotropic gradient energy coefficient, its derivative and square
  scalarvalueType W   = constV(W0) * (constV(1.0) + constV(epsilonM) * cos_6_theta);
  ...
  [[maybe_unused]] Point<dim, VectorizedArray<double>> q_point_loc) const
{
  ...
  // 6-fold sin(theta)
  scalarvalueType sin_6_theta =
    -constV(36.0) * n[1] * n[0] +
    constV(192.0) * dealii::Utilities::fixed_power<3>(n[1]) * n[0] -
    constV(192.0) * dealii::Utilities::fixed_power<5>(n[1]) * n[0];

  // Anisotropic gradient energy coefficient, its derivative and square
  scalarvalueType W       = constV(W0) * (constV(1.0) + constV(epsilonM) * cos_6_theta);
  scalarvalueType W_theta = constV(-W0) * (constV(epsilonM) * sin_6_theta);
