...
public:
    ...
    void setInitialCondition(const dealii::Point<dim>& p,
        const unsigned int index, double& scalar_IC, dealii::Vector<double>& vector_IC);
    ...
    void setNonUniformDirichletBCs(const dealii::Point<dim>& p,
        ...
        const double time, double& scalar_BC,
...
#include "../../include/typeDefs.h"
    ...
    void equationLHS(variableContainer<dim, degree,
                         dealii::VectorizedArray<double>>& variable_list,
...
// Not useful for this application
    ...
    double getNucleationProbability(variableValueContainer
                                        variable_value,
        double dV) const;
#endif
