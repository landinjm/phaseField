...
template <int dim, int degree>
void Checkpoint<dim, degree>::load_checkpoint_triangulation()
{
...
template <int dim, int degree>
void Checkpoint<dim, degree>::load_checkpoint_time_info()
{
    ...
    tStepRef.currentIncrement = dealii::Utilities::string_to_int(line);

    std::getline(time_info_file, line);
    line.erase(line.end() - 14, line.end());
    tStepRef.currentTime = dealii::Utilities::string_to_double(line);
    time_info_file.close();
}

// Load from a previously saved checkpoint
template <int dim, int degree>
void Checkpoint<dim, degree>::load_checkpoint_fields()
{
...
template <int dim, int degree>
void Checkpoint<dim, degree>::verify_checkpoint_file_exists(const std::string filename)
{
    std::ifstream in(filename);
    if (!in) {
        AssertThrow(false,
            ExcMessage(std::string("PRISMS-PF Error: You are trying to restart a previous computation, " "but the restart file <")
                + filename
